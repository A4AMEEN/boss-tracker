...
export class TripService {
  ...
  /** Auto-calculate inUAEDays (exact) and suggest inIndiaDays (editable) */
  calculateDays(params: {
    username: string;
    travelDate?: string;
    returnDate?: string;
    tripId?: string;
  }): Observable<{ success: boolean; data: { inUAEDays: number; inIndiaDays: number } }> {
    let p = new HttpParams();
    Object.entries(params).forEach(([k, v]) => { if (v) p = p.set(k, v); });
    return this.http.get<{ success: boolean; data: { inUAEDays: number; inIndiaDays: number } }>
      (`${this.apiUrl}/trips/calculate-days`, { params: p });
  }
  ...
  getStats(year?: string, username?: string): Observable<{ success: boolean; data: Stats }> {
    ...
    return this.http.get<{ success: boolean; data: Stats }>
      (`${this.apiUrl}/stats/summary`, { params });
  }
